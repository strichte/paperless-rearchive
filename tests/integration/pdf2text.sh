#!/bin/bash
# Dump the full pdftotext output for a document's archive and/or original version.
#
# Companion to check_archive_provenance.sh: instead of just the Creator line,
# this prints the *complete* pdftotext output (pages, page size, producer,
# Creator, file size, ...) so the two versions of the same document can be
# compared by eye.
#
# Usage:
#     tests/integration/pdftotext_compare.sh -a <doc_id> [...]   # archive only
#     tests/integration/pdftotext_compare.sh -o <doc_id> [...]   # original only
#     tests/integration/pdftotext_compare.sh <doc_id> [...]      # both versions
#     tests/integration/pdftotext_compare.sh -o --doc 4221 --doc 4222
#     tests/integration/pdftotext_compare.sh /abs/path.pdf [...] # raw path
#
...
# ---------------------------------------------------------------------------
# Inner script: runs inside the *paperless* container.  Only needs pdftotext.
...
command -v pdftotext >/dev/null || { echo "pdftotext not found in container" >&2; exit 1; }
...
dump() {
  ...
  if ! pdftotext "$target" - ; then
    printf 'ERROR     (pdftotext failed)\n\n'
    return 1
  fi
  printf '\n'
}
