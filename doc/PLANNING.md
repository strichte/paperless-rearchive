...
| `REARCHIVE_PDF_PROVENANCE` | `on` | per-page born-digital detection (pdf-inspector): `on` classifies each PDF page with pdf-inspector and routes pages to OCR; `off` falls back to paperless-ngx's document-level heuristics and keeps mode-driven behaviour. See §8 |
