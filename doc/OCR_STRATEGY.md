...
> * **P4** — done. `ocr/runner.py` demoted to integration-harness-only (the active pipeline uses `ingest_args.py`); the dead `getattr(settings, 'ocr_dpi', 300)` became a real `REARCHIVE_OCR_DPI` knob (content-only render DPI); poller lost-wakeup fixed (`_sleep_or_immediate`, regression-tested); README diagram/HUP note + PLANNING risks updated; `unpaper` added to the image for `--clean`.
